...
RAW = os.path.join(HERE, '_ofrenda', '_raw')
OUT = os.path.join(HERE, '_ofrenda', 'master')
TMP = os.path.join(HERE, '_ofrenda', '_tmp')
...
SET = [
    ...
    ('Cooling_The_Soil.mp3',      'MONZÓN',
     'la lluvia que llega toda de golpe después de meses'),
    ('Glow_in_the_Deep.mp3',      'BIOLUMINISCENCIA',
    ...
    ('Salt_Flat_Mirror.mp3',      'SALAR',
     'el salar bajo el agua: tierra y cielo la misma cosa. Sin voz'),
    ('Between_The_Tides.mp4',     'CENIZA',
     'lo que queda de la ofrenda; cierra el círculo'),
]
...
def build():
    ...
    crudo_set = os.path.join(TMP, 'ofrenda-set-crudo.wav')
...
if __name__ == '__main__':
    ...
    meta = dict(
        ...
        keys=TONOS, offsets=cortes, file='audio/amr-ofrenda.m4a',
        art='art/amr-ofrenda.svg', edition=8, peaks=pk, bpm=120, key='F MAJ')
    with open(os.path.join(HERE, 'ofrenda.js'), 'w') as f:
        f.write('window.AMR_OFRENDA=' + json.dumps(meta, ensure_ascii=False) + ';')
    print('ofrenda.js escrito ·', len(SET), 'cortes')
