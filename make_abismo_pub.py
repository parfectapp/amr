#!/usr/bin/env python3
"""Publica ABISMO: m4a + abismo.js + cortes."""
import os, json, subprocess
import numpy as np, imageio_ffmpeg
from dream_core import ffmeter, master_file, ffdecode, wav_write, sat
from make_abismo import SECTIONS, TMP, SPB, BPM

FF = imageio_ffmpeg.get_ffmpeg_exe()
HERE = os.path.dirname(os.path.abspath(__file__))
SRC = os.path.join(HERE, 'masters', 'amr-abismo.wav')
...
if __name__ == '__main__':
    m4a = os.path.join(HERE, 'audio', 'amr-abismo.m4a')
    enc(SRC, m4a)
    ...
    meta = dict(id='amr-abismo', title='ABISMO', kicker='MELODIC TECHNO · DRUMS REALES',
                tracks=len(titles), dur=round(acc, 1), titles=titles, offsets=offsets,
                file='audio/amr-abismo.m4a', art='art/shots/shot-abismo.svg', edition=12,
                peaks=peaks(m4a), bpm=int(BPM), key='A MIN')
    with open(os.path.join(HERE, 'abismo.js'), 'w') as f:
        f.write('window.AMR_ABISMO=' + json.dumps(meta) + ';')
    print('abismo.js escrito', flush=True)
    ...
    for i, s in enumerate(SECTIONS):
        ...
        dst = os.path.join(HERE, 'audio', f'amr-abismo-cut-{s["name"].lower()}.m4a')
