#!/usr/bin/env python3
"""TAMBOR — afro house melodic: el tambor manda (~42 min, 125 BPM). Motor v4.
...
def hit_djembe(rng, art='t'):
    """djembe con 3 articulaciones: t=tono (90-120Hz), s=slap (agudo seco), b=bajo (60-80Hz)."""
    if art == 'b': f0, dec, sl = rng.uniform(58, 78), 0.22, 0.25
    elif art == 's': f0, dec, sl = rng.uniform(180, 240), 0.05, 0.9
    else: f0, dec, sl = rng.uniform(90, 125), 0.13, 0.45
    n = int(dec * 6 * SR); t = np.arange(n) / SR
    x = np.sin(2 * np.pi * f0 * (1 + 0.3 * np.exp(-t / 0.008)) * t) * np.exp(-t / dec)
    slap = rng.standard_normal(n).astype(np.float32) * np.exp(-t / 0.004) * sl
    return sat((x + bp(slap, 1200, 6500, 2)).astype(np.float32), 1.6, 0.1) * rng.uniform(0.85, 1.0)

def hit_talking(rng, up=True):
    """talking drum: el tono GLISSA (habla) — la voz del tambor."""
    n = int(0.28 * SR); t = np.arange(n) / SR
    f = (110.0 * (2 ** (t / 0.28 * 0.8))) if up else (190.0 * (0.5 ** (t / 0.28 * 0.7)))
    x = np.sin(2 * np.pi * np.cumsum(f) / SR) * np.exp(-t / 0.12)
    x += 0.3 * rng.standard_normal(n) * np.exp(-t / 0.005)
    return sat(x.astype(np.float32) * 0.8, 1.5, 0.1)

def hit_cowbell(rng):
    n = int(0.11 * SR); t = np.arange(n) / SR
    x = (np.sign(np.sin(2 * np.pi * 545 * t)) * 0.6 + np.sign(np.sin(2 * np.pi * 812 * t)) * 0.4)
    x = x * np.exp(-t / 0.045)
    return bp(x.astype(np.float32), 400, 3200, 2) * 0.5 * rng.uniform(0.85, 1.0)
...
SECTIONS = [
 dict(name='LLAMADO', root=45, sc=NAT, energy=0.42, shape='rise', bars=88, bass='round',
      chords=[[45,52,60,64],[41,48,57,60]],
      motif=[(0,4,1,4),(8,2,1,3),(16,4,1,2),(20,7,0,6)]),
 dict(name='CUERO', root=45, sc=DOR, energy=0.58, shape='wave', bars=112, bass='round',
      chords=[[45,52,60,62],[50,57,65,69],[45,52,60,62],[43,50,58,62]],
      motif=[(0,0,1,2),(4,2,1,2),(8,4,1,3),(16,2,1,2),(22,0,1,5)]),
 dict(name='PALMAS', root=40, sc=NAT, energy=0.66, shape='wave', bars=112, bass='round',
      chords=[[40,47,55,59],[45,52,60,64],[36,43,52,55],[43,50,58,62]],
      motif=[(0,4,1,1),(2,4,1,1),(4,5,1,2),(8,4,1,2),(12,2,1,3),(16,4,1,2),(20,7,0,4)]),
 dict(name='CANTO', root=43, sc=DOR, energy=0.78, shape='wave', bars=120, bass='round',
      chords=[[43,50,58,62],[48,55,64,67],[43,50,58,62],[41,48,57,60]],
      motif=[(0,4,1,2),(3,5,1,1),(4,4,1,2),(8,2,1,2),(12,0,1,3),(16,4,1,2),(20,7,0,3),(26,4,1,4)]),
 dict(name='LUNA', root=38, sc=NAT, energy=0.50, shape='deep', bars=96, bass='round',
      chords=[[38,45,53,57,60],[34,41,50,53]],
      motif=[(0,4,1,6),(8,3,1,4),(16,2,1,6),(24,0,1,8)]),
 dict(name='RITUAL', root=45, sc=NAT, energy=0.95, shape='peak', bars=144, bass='round',
      chords=[[45,52,60,64],[48,55,64,67],[41,48,57,60],[43,50,58,62]],
      motif=[(0,4,1,1),(2,4,1,1),(4,7,0,3),(8,5,1,2),(12,4,1,2),(16,2,1,1),(18,4,1,1),(20,7,0,3),(26,5,1,4)]),
 dict(name='TIERRA', root=43, sc=DOR, energy=0.72, shape='wave', bars=112, bass='round',
      chords=[[43,50,58,62],[40,47,55,59],[38,45,53,57],[40,47,55,59]],
      motif=[(0,4,1,3),(6,5,1,2),(10,4,1,2),(16,2,1,3),(22,4,1,5)]),
 dict(name='MADRUGADA', root=45, sc=NAT, energy=0.45, shape='outro', bars=80, bass='round',
...
def render_section(sec, idx):
    ...
    for bi, b in enumerate(blocks):
        ...
        for bar in range(8):
            ...
            if pg > 0:
                DJ = [
                    [(0,'t',0.6),(3,'s',0.9),(6,'t',0.8),(8,'b',0.7),(11,'s',1.0),(14,'t',0.8)],
                    [(2,'t',0.8),(5,'s',0.9),(7,'t',0.6),(10,'s',1.0),(12,'b',0.7),(15,'t',0.6)],
                    [(0,'b',0.8),(3,'s',0.9),(7,'t',0.7),(10,'t',0.8),(13,'s',1.0)],
                ]
                thin = last16 and (gb // 16) % 2 == 1
                for st, art, v in DJ[pvar]:
                    if thin and st >= 8: continue
                    if rng.uniform() < 0.9:
                        pos = base + int(sw(st, 'conga') + rng.normal(0, 0.005) * SR)
                        add(drumb, pos, hit_djembe(rng, art), pg * v * 0.55)
                if bar % 2 == 0:
                    for st in (0, 3, 6, 10, 12):
                        add(drumb, base + int(sw(st, 'conga')), hit_rim(rng), pg * 0.26)
                if b['gain'] >= 0.85 and bar % 2 == 1:
                    for st in (2, 10):
                        add(drumb, base + int(sw(st, 'hats')), hit_cowbell(rng), pg * 0.4)
                if bar % 8 == 7:
                ...
                if blockend and pg > 0.3:
                    for k, st in enumerate((12, 12.5, 13, 13.5, 14, 14.5, 15)):
                        ...
                        add(drumb, pos, hit_djembe(rng, 's' if k % 2 else 't'), pg * (0.3 + 0.07 * k))
            # --- shaker en 12/8 CRUZADO sobre el 4/4 (el vaivén afro)
            if b['shaker']:
                if not (last16 and (gb // 16) % 2 == 0):
                    for k in range(12):
                        pos = base + int(k * SPB / 12 + rng.normal(0, 0.003) * SR)
                        acc = 1.0 if k % 3 == 0 else (0.5 if k % 3 == 1 else 0.7)
                        add(drumb, pos, hit_shaker(rng), 0.17 * acc)
            # --- hats mínimos (el aire lo dan los cueros)
            hv = b['hats']
            if hv > 0:
                for st in (2, 6, 10, 14):
                    add(drumb, base + int(sw(st, 'hats') + rng.normal(0, 0.003) * SR), hit_hat(rng), hv * 0.32)
                if bar % 4 == 3:
                    add(drumb, base + int(sw(8, 'hats')), hit_hat(rng, open_=True), hv * 0.3)
            # --- CANTO call-response (la voz del ritual)
            if b['gain'] >= 0.8 and sec['name'] in ('CANTO', 'RITUAL', 'TIERRA'):
                import colibri_voices as CV
                if bar % 4 == 0:
                    for st, d2, vow, ln in [(0, 0, 'o', 3), (6, 2, 'e', 2)]:
                        add(keysb, base + int(sw(st, 'keys')), CV.vocal_la(midi_f(deg(root, sc, d2, 0)), ln * S16 / SR * 1.2, rng, vow), 0.55)
                elif bar % 4 == 2:
...
def build(only=None):
    ...
    print(f'TAMBOR · {len(SECTIONS)} secciones · {total_bars} compases ≈ {total_bars*2/60:.0f} min')
    secs = []
    ...
    for k, (i, s, f) in enumerate(secs):
        ...
        pos += s['bars'] * SPB
        del x
    raw = os.path.join(TMP, 'tambor-raw.wav')
    wav_write(raw, out)
    del out
    print('  … masterizando a -8 LUFS', flush=True)
    final = os.path.join(HERE, 'masters', 'amr-tambor.wav')
    os.makedirs(os.path.join(HERE, 'masters'), exist_ok=True)
    hist = master_file(raw, final, target_i=-9.5, ceiling_db=-1.1)
