#!/usr/bin/env python3
"""MAREA — set hipnótico melodic/organic tropical (~45 min, 120 BPM).
...
TMP = os.path.join(HERE, '_marea_tmp')
...
SWING = 0.55                               # swing MPC-ish en semicorchea
...
def bass_note(f, dur, rng, mode='round', fc=760.0):
    ...
    if mode == 'round':
        ...
        x += 0.5 * _saw(np.full(n, f, np.float32), rng.uniform())
...
SECTIONS = [
 dict(name='SEMILLA', root=40, sc=NAT, energy=0.42, shape='rise', bars=88, bass='round',
      chords=[[40,47,55,59,62],[36,43,52,59,64]],
      motif=[(0,4,1,3),(6,2,1,2),(10,0,1,4),(16,4,1,3),(22,5,1,2),(26,4,1,5)]),
 dict(name='RAIZ', root=40, sc=DOR, energy=0.55, shape='wave', bars=128, bass='roll',
      chords=[[40,47,55,62],[45,52,60,64],[40,47,55,62],[38,45,53,57]],
      motif=[(0,0,2,2),(3,2,2,2),(6,4,2,2),(10,3,2,3),(16,4,2,2),(19,2,2,2),(22,0,2,6)]),
 dict(name='RAMA', root=45, sc=NAT, energy=0.62, shape='wave', bars=136, bass='roll',
      chords=[[45,52,60,64],[41,48,57,60],[45,52,60,64],[43,50,58,62]],
      motif=[(0,4,1,2),(4,4,1,2),(8,5,1,2),(11,4,1,2),(16,2,1,3),(20,0,1,2),(24,2,1,6)]),
 dict(name='FLOR', root=45, sc=DOR, energy=0.70, shape='wave', bars=144, bass='roll',
      chords=[[45,52,60,66],[50,57,65,69]],
      motif=[(0,0,2,1),(2,2,2,1),(4,4,2,2),(8,6,1,2),(12,4,2,2),(16,0,2,1),(18,2,2,1),(20,4,2,2),(24,3,2,4)]),
 dict(name='SOMBRA', root=38, sc=DOR, energy=0.55, shape='valley', bars=120, bass='round',
      chords=[[38,45,53,57,62],[34,41,50,57]],
      motif=[(0,4,1,4),(8,3,1,3),(14,2,1,2),(16,4,1,4),(24,5,1,6)]),
 dict(name='ABRIL', root=41, sc=NAT, energy=0.76, shape='rise', bars=144, bass='roll',
      chords=[[41,48,56,60],[45,52,60,64],[36,43,52,58],[43,50,58,62]],
      motif=[(0,4,2,2),(3,5,2,1),(4,4,2,2),(8,2,2,2),(12,0,2,3),(16,4,2,2),(19,5,2,1),(20,7,1,4),(26,4,2,4)]),
 dict(name='CALMA', root=45, sc=NAT, energy=0.48, shape='deep', bars=104, bass='round',
      chords=[[45,52,60,63,67],[40,47,55,60]],
      motif=[(0,2,1,6),(8,0,1,4),(16,3,1,6),(24,2,1,6)]),
 dict(name='LLUVIA', root=45, sc=NAT, energy=0.97, shape='peak', bars=160, bass='roll',
 ...
 dict(name='PETALOS', root=43, sc=NAT, energy=0.78, shape='wave', bars=128, bass='roll',
      chords=[[43,50,58,62],[40,47,55,59],[38,45,53,57],[40,47,55,59]],
      motif=[(0,4,1,3),(6,5,1,2),(10,4,1,2),(16,2,1,3),(22,4,1,2),(26,5,1,5)]),
 dict(name='PRIMAVERA', root=40, sc=NAT, energy=0.52, shape='outro', bars=88, bass='round',
      chords=[[40,47,55,59,64],[36,43,52,59]],
...
def build(only=None):
    ...
    for k, (i, s, f) in enumerate(secs):
        ...
        pos += s['bars'] * SPB
        del x
    raw = os.path.join(TMP, 'marea-raw.wav')
    wav_write(raw, out)
    del out
